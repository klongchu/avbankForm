...
function notifyInfo(data) {
    ...
    if (data != '') {
        ...
        var content = {};
        var color = "";
        content.title = Bundles['txt.script.noti.title.info'];
...
function notifyDelComment(url, id) {
    ...
    var content = {};
    content.icon = 'icon ' + 'la la-info-circle';
    content.title = Bundles['txt.script.noti.title.confirm'];
    content.message = Bundles['btn.delete'];
...
function notifyConfirm(url, id) {
    ...
    var content = {};
    content.title = Bundles['txt.script.noti.title.confirm'];
    content.message = Bundles['txt.script.noti.msg.confirm'];
    content.icon = 'icon ' + 'la la-info-circle';
...
// +
                '&nbsp;&nbsp;&nbsp;&nbsp; <button style="background-color: #f44e16;color: white;" class="btn info" onclick=\'javascript:callAjaxRefreshTableMain(\"' + url + '\",\"' + id + '\");closeNoti();\'>' + Bundles['txt.script.confirm'] + '</button></span>' +
                '<div>' +
                '</div>' +
                '</div>'
    });
...
function notifyError(errorCode) {
    $.ajax({
        type: 'GET',
        url: contextPath + '/api/Common/inquiryError/' + errorCode,
        dataType: 'json',
        beforeSend: function () {
        },
        success: function (data) {
...
function initialUpdate(index, url, id) {
    var sUpdate = (typeof update !== "undefined" ? update : "");
    var sSearch = (typeof search !== "undefined" ? search : "");

    if (!isActive(sUpdate) && !isActive(sSearch)) {
        return '';
    }
